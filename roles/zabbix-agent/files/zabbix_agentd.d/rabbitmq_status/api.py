...
class RabbitMQAPI(object):
    ...
    def check_overview(self, item):
        ...
        elif item in [ 'messages_rate','messages_ready_rate','messages_unacknowledged_rate' ]:
            item = item.replace('_rate','_details')
            return self.call_api('overview').get('queue_totals').get(item).get('rate',0)
        elif item in [ 'ack','deliver','deliver_get','publish','redeliver' ]:
            return self.call_api('overview').get('message_stats').get(item,0)
        elif item in [ 'ack_rate','deliver_rate','deliver_get_rate','publish_rate','redeliver_rate' ]:
            item = item.replace('_rate','_details')
            return self.call_api('overview').get('message_stats').get(item).get('rate',0)
        elif item == 'rabbitmq_version':
            return self.call_api('overview').get('rabbitmq_version', 'None')

    def check_cluster(self, item):
        ...
        for node in nodes:
            if node['running']:
                ...
                fail_nodes.append(node['name'])
        if item == 'failure':
            return ','.join(fail_nodes)
        elif item == 'nodes':
            return running_num 
...
def main():
    ...
    elif options.check == 'cluster':
        print api.check_cluster(options.metric)
